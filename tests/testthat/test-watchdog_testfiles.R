...
testthat::test_that("T1.1 executed.",{
  ...
  apply(last_change_df, 1, function(x) {
    ...
    last_update_by_old <- regmatches(
      x = file_content,
      m = gregexpr("section Last updated by(.*)", file_content, perl = TRUE)
    )
    last_update_by_old <- gsub(
      pattern = "section Last updated by: ",
      replacement = "",
      x = last_update_by_old
    )

    last_update_when_old <- regmatches(
      x = file_content,
      m = gregexpr("section Last update date(.*)", file_content, perl = TRUE)
    )
    last_update_when_old <- gsub(
      pattern = "section Last update date: ",
      replacement = "",
      x = last_update_when_old
    )

    if (!(
      x["last_edit_by"] == last_update_by_old |
      x["last_edit_when"] == last_update_when_old
    )) {
