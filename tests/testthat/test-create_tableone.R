...
test_that("summary numeric - overall", {
  ...
  expect_equal(testfun_mean(x), as.character(values[1,]))
  expect_equal(testfun_median(x), as.character(values[2,]))
  expect_equal(testfun_range(x), as.character(values[3,]))
...
test_that("summary numeric - strata", {
  ...
  expect_equal(testfun_mean(x_1), as.character(values_s1[1,]))
  expect_equal(testfun_median(x_1), as.character(values_s1[2,]))
  expect_equal(testfun_range(x_1), as.character(values_s1[3,]))
  
  x_2 <- dat_test[dat_test$strat == "s2", "dose"]
  values_s2 <- t2[t2$variable == "dose", "s2"]
  expect_equal(testfun_mean(x_2), as.character(values_s2[1,]))
  expect_equal(testfun_median(x_2), as.character(values_s2[2,]))
  expect_equal(testfun_range(x_2), as.character(values_s2[3,]))
