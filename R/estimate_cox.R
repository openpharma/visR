...
#' @param ... additional arguments passed on to the ellipsis of the call \code{survival::coxph(data = data, formula = Surv(AVAL, 1-CNSR) ~ main), ...)} .
...
estimate_cox <- function(
  ...
  formula <- stats::as.formula(glue::glue(paste0("Surv(", AVAL, ", 1-", CNSR, ") ~ {main}")))
