...
#' @param ptype Character vector containing the type of test for the p-value. Current options are "Log-Rank", "Wilcoxon", "Custom", "All".
#'    "Custom" allows the user to specify the weights on the Kaplan-Meier estimates using the argument `rho`. When `rho` is specified in context of "All",
#'    the G-rho family of Harrington and Fleming is displayed.
#'    The default is "All" displaying all types possible.
#'@param rho Scalar parameter which controls the type of test. `rho=0` is equivalent to the Mantel-Haenszel test (log-rank test) and `rho=1` to the
...
get_pvalue.survfit <- function(survfit_object,
  ...
  if (!base::any(c("Log-Rank", "Wilcoxon",  "Custom", "All") %in% ptype))
