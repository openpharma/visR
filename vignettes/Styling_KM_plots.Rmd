...
In this example, we will work with patient data from NCCTG Lung Cancer dataset that is part of the `survival` package. This data is also used to demonstrate more functions of `visR` in the vignette XXX. However, in this particular one, it will only be used to demonstrate the adjustments of the aesthetics. 
...
lung_suvival_object %>%
  visR::visr(x_label = "Time",
             ...
             legend_position = "top") 
```
However, these rather minimal adjustments usually don't cover all the things a user wants to modify. Therefore, we provide two additional function to adjust more aesthetics: `visR::define_theme()` and `visR::apply_theme()`. The first one provides an easy wrapper to create a nested list of list with styling options that is then applied to the plot by the second function.
...
```{r visr-apply_theme-non-empty-fancy}
lung_suvival_object %>%
  visR::visr() %>%
  visR::apply_theme(theme) %>%
  visR::add_CI() %>%
