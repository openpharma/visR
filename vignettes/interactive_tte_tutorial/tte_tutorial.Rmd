...
![](images/package_architecture_overview.png)

In principle, `extimate_XXX` functions allow to estimate statistical models from the data (e.g. `estimate_KM` estimates a survival model), `get_XXX` functions are getters for all kinds of summary statistics (e.g. `get_tableone` or `get_pvalue`), `render` and `visr` display plots and tables, and `add_XXX` functions add specific information to a plot or table (e.g. `add_CI` adds confidence intervals to a plot.)

### Example

![](images/package_architecture_example.png){width=90%}
...
age_stay_scatterplot = data %>%
  ggplot2::ggplot(ggplot2::aes(x = Age,
                               y = Stay)) + 
    ggplot2::geom_bin2d(bins = c(50,25)) +
...
As mentioned, [`visR`](https://openpharma.github.io/visR/) has defaults following the CDISC ADaM standard. If your data set uses these as well, estimating and plotting survival curves is particularily straightforward.
...
**Exercise 1**: Add information about statistcal significance tests to a survival plot. (Hint: Combine `add_annotation()` and `get_pvalue()`).
...
A second release of [`visR`](https://openpharma.github.io/visR/) to CRAN is planned for October. It will include a few bugfixes and UX improvements. However, it will also add new features:

* `add_highlight()` -> Adjusts the transparency of strata in the survival plots to highlight specific ones.

![](images/add_highlight.png)


* `tabletwo()` -> A set of functions to generate Table 2 statistics with forest plots.

![](images/table_two.png){width=90%}
