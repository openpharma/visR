...
```{r}
cohort_attrition %>% gt::gt() %>% 
  gt::tab_options(data_row.padding = gt::px(4)) %>% 
  gt::fmt_number(c("Remaining N", "Remaining %", "Excluded %"), decimals = 2, sep_mark=',')
```
...
# render table
tab1_groups %>% vr_render_tableone(title = "BRCA patients", 
...
km_summary[[1]] %>% 
  gt::gt() %>% 
  gt::fmt_number(c("median survival time", "0.95LCL", "0.95UCL"), decimals = 2)

km_summary[[2]] %>% 
  gt::gt() %>% 
  gt::fmt_number(c("statistic", "p.value"), decimals = 2)
```
